...
| Export delivery          | `export?format=html` returns 33.9 MB of gzip data with no content type, which Node's `fetch` leaves compressed. Decompressed, it is the same 45 MB HTML page with embedded PNG images. The download took about 28 seconds. Corrected 2026-09-26: Google does send `content-encoding: gzip` and a content type. jsdom's undici 8 made Node's built-in `fetch` drop every header over HTTP/2. The importer now uses undici's own `fetch` and receives decoded HTML. |
