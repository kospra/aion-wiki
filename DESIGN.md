...
- `ImageViewer` remains a clickable image with an expand affordance and a simple modal: full-size original and close button. Chakra owns focus trapping, Escape and focus restoration.
- Figure annotations follow the shared [Figma component 24:655](https://www.figma.com/design/M1BKIOXsv2IJvTEbK4Cj1t/aion-wiki?node-id=24-655): Chakra Badge marker, optional color square, and the explanation as the title, followed by exact screenshot detail and spaced source links. The shared `wiki.annotationTitle` style uses semibold 18px mobile / 20px desktop text with 1.4 line height. Render the explanation once. Use one vertical flow at all widths, 24px row padding and 12px content gaps. Preserve every source destination and uncertainty qualifier. `GuideFigure` owns this pattern across all articles.
- Verified screenshot annotations include a 12×12px filled square swatch between the marker and title. `wiki.annotation.*` tokens in `theme.ts` preserve sampled source border colors; `GuideFigure` maps them explicitly by figure and label, keeping secondary item/text colors distinct. With a verified swatch, retain the written color locator as visually hidden accessible context. Without a verified swatch, keep it visible beneath the title.
